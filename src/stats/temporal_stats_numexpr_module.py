...
def temporal_stats(array, no_data=None, as_bip=False):
    ...
    if as_bip:
        ...
        t_emp = numexpr.evaluate("(residuals / stddev)**3",
                                 {'stddev': stats[4],
                                  'residuals': residuals}).transpose(1,
                                                                     2,
                                                                     0)
        nan_sum = numpy.nansum(t_emp, axis=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=False,
        type=bool,
        help='If set to True, the array will be transposed to be [Lines,Samples,Bands], and processed in this fashion. The default is to process the array as [Bands,Lines,Samples].')
