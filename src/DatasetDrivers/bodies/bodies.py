...
class Satellite(object):
    ...
    def __init__(self, sat_name, sensor):
        ...
        def parse_list(list_string, element_type=str):
            ...
            return [
                element_type(
                    element.strip()) for element in re.search(
                    '\s*(?<=\[)(.*)(?=\])\s*',
                    list_string).group(0).split(',')]
        ...
        def get_sensor(sat_node, sensor):
            ...
            def get_bands(sensor_node):
                ...
                for band_node in sensor_node.getElementsByTagName('BAND'):
                    ...
                    band_dict['WAVELENGTH'] = [
                        float(wavelength) for wavelength in unicode_to_ascii(
                            band_node.getAttribute('WAVELENGTH')).split('-')]
            ...
            self.rgb_bands = [
                int(bandstring) for bandstring in unicode_to_ascii(
                    sensor_node.getAttribute('RGB_BANDS')).split(',')]
            self.acquistion_seconds = timedelta(
                seconds=float(
                    unicode_to_ascii(
                        sensor_node.getAttribute('ACQUISITION_SECONDS'))))
    ...
    def _load_tle_from_file_sequence(self, centre_datetime, data_root=None, tle_search_range=45):
        ...
        if os.path.exists(tle_path):
            try:
                ...
                return open_tle(tle_path, centre_datetime)
            except (Exception) as e:  # TODO: Tighten up this except clause - too general
