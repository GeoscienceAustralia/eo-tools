...
class Dataset(gdal.Dataset):
    ...
    def get_pixel_indices(self, longlat=(0.0, 0.0), nBand=1):
        ...
        assert x in range(
            0, self.RasterXSize), 'Pixel X value %d out of range (0 - %d) for longitude %f' % (x, self.RasterXSize, longlat[0])
        assert y in range(
            0, self.RasterYSize), 'Pixel Y value %d out of range (0 - %d) for latitude %f' % (y, self.RasterYSize, longlat[1])
